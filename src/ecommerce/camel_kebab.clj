...
;;;; sub-dependency tree), lessened build non-determinism, a simplified
;;;; interface, and customizations that combine the existing tree walk with
;;;; :_id key preservation (something that would otherwise require an
;;;; additional walk post-conversion).

(ns ecommerce.camel-kebab
...
(defn convert-case [first-fn rest-fn sep s]
  (if (= s "_id") s
      (let [[first & rest] (split s)]
        (str/join sep (cons (first-fn first) (map rest-fn rest))))))
...
(defconversion "camelCase" str/lower-case str/capitalize "")
(defconversion "kebab-case" str/lower-case str/lower-case "-")
